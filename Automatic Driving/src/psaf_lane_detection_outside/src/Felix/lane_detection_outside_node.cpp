...
void LaneDetectionOutsideNode::extractLaneMarkings(const cv::Mat& img, cv::Mat& debugImage){
    ...
    int sumNonZeroCenter = 0, sumNonZeroRight = 0, windowCount = 0, emptyCenterWindows = 0;
    ...
    if (startpoint.x!= 0||startpoint.y!= 0) {
        ...
        for (int y = startpoint.y; y > 100; y -= 15) {
            if (bpInfo.searchC) {
...
//                }
    ...
    if (second_startpoint.x!=0 ||second_startpoint.y!= 0) {
        ...
        for (int y = second_startpoint.y; y > 100; y -= 15) {
            if (bpInfo.searchC) {
...
std::vector<cv::Point> LaneDetectionOutsideNode::getBasePoints(const cv::Mat& img, cv::Mat& debugImage){
    ...
    for (const auto& contour : contours) {
        ...
        if (area >= minArea && area <= maxArea) {
            ...
            maxX = *std::max_element(contourXCoords.begin(), contourXCoords.end());
            maxY = *std::max_element(contourYCoords.begin(), contourYCoords.end());
            minX = *std::min_element(contourXCoords.begin(), contourXCoords.end());
            minY = *std::min_element(contourYCoords.begin(), contourYCoords.end());
...
//    }
    ...
    for (int x = 320; x < img.cols - 140; x += 64) {
        ...
        non_zero_count = cv::countNonZero(subWindow);
