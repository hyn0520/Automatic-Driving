...
//    }
    ...
    for (int x = 50; x < img.cols - 140; x += 64) {
        cv::Rect hWindow(x, 360, 64, 70);
        ...
        if (valid) {
            ...
            if (lastBasePoint == cv::Point() || std::abs(basePoint.x - lastBasePoint.x) >= 120) {
                if (startpoint.x != 0||startpoint.y !=0) {
                    if (basePoint.x >= (startpoint.x - 70) && basePoint.x < 500) {
